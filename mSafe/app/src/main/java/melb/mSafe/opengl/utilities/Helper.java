...
public class Helper {
	...
	private static float[] copyMatrix(float[] matrix) {
        ...
        System.arraycopy(matrix, 0, copyM, 0, matrix.length);
		return copyM;
	}

	public static float[] animateObject(float[] animateResult, float[] mvpMatrix, boolean aroundCenter, float x, float y, float z) {
		float[] newMatrix = copyMatrix(mvpMatrix);
		if (animateResult != null) {
			Matrix.translateM(newMatrix, 0, animateResult[2], animateResult[3],
					animateResult[4]);
			rotateModel(newMatrix, null, null, animateResult[0], aroundCenter, x,
                    y, z);
		}
		return newMatrix;
	}
